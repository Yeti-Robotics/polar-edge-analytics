...
export default async function TeamData({ searchParams }: { searchParams: Promise<{ id: string }> }) {
		...
		<div className="flex flex-col">
			...
			<div className="my-4">
				{!teamDataResult.success
					...
					: !tournamentData.success ? "Error fetching tournament data" : teamDataResult.value.length < 1
						? "No team data yet"
						: ""}
			</div>
			<div>
				{teamDataResult.success && tournamentData.success && (
					<TeamDataTable teamData={teamDataResult.value} tournaments={tournamentData.value} />
				)}
			</div>
		</div>
	);
